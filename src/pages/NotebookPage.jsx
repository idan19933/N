...
const NotebookPage = () => {
    ...
    const loadNotebookData = async () => {
        try {
            ...
            // Fetch entries
            const entriesRes = await fetch(`http://localhost:3001/api/notebook?userId=${user.id}`);
            const entriesData = await entriesRes.json();

            // Fetch stats
            const statsRes = await fetch(`http://localhost:3001/api/notebook/stats?userId=${user.id}`);
    ...
    const filteredEntries = filter === 'all'
        ? entries
        : entries.filter(e => e.topic === filter);
            ...
            <div className="max-w-6xl mx-auto mb-8">
                ...
                >
                    ...
                    <p className="text-gray-200 text-lg">
                        כל התרגילים והנושאים שפתרת במקום אחד
                    </p>
                </motion.div>
            ...
            <div className="max-w-6xl mx-auto">
                {filteredEntries.length === 0 ? (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="bg-white rounded-xl p-12 text-center shadow-xl"
                    >
