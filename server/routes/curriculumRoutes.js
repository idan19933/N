    ...
    try {
        ...
        const result = await pool.query(`
            SELECT
                ...
                ROUND(
                        AVG(CASE WHEN attempts > 0 THEN (correct_attempts::float / attempts * 100) ELSE 0 END)::numeric,
                        1
                ) as accuracy,
                ROUND(
                        (SUM(CASE WHEN correct_attempts > 0 THEN 1 ELSE 0 END)::float / COUNT(*) * 100)::numeric,
                        1
                ) as progress_percent,
